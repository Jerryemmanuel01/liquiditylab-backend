...
export const sendEmail = async (options: EmailOptions) => {
  ...
  const mailOptions = {
    from: 'Liquidity Lab Support <support@liquiditylab.io>',
    to: options.email,
    subject: options.subject,
    text: options.message,
    // html: ... (can add HTML template here)
  };

  // 3) Actually send the email
  // If we don't have proper credentials, just log it so development isn't blocked.
  if (process.env.NODE_ENV === 'development' && (!process.env.EMAIL_USERNAME || process.env.EMAIL_USERNAME === 'development_user')) {
    console.log('\n================== EMAIL MOCK ==================');
